    ...
    if [ "${group_as}" != "IXP" ];then
        ...
        for ((i=0;i<n_routers;i++)); do
            ...
            if [ "${property1}" = "BGP_MONITOR"  ];then
                ...
                echo -n "-- add-br ${group_k}_artemis " >> "${DIRECTORY}"/groups/add_bridges.sh
                echo "ip link set dev ${group_k}_artemis up" >> "${DIRECTORY}"/groups/ip_setup.sh

                subnet_bridge="$(subnet_router_EXABGP_MONITOR "${group_number}" "bridge")"
                subnet_artemis="$(subnet_router_EXABGP_MONITOR "${group_number}" "monitor")"
                subnet_group="$(subnet_router_EXABGP_MONITOR "${group_number}" "group")"

                ./setup/ovs-docker.sh add-port ${group_k}_artemis group_"${group_number}" ${group_k}_ARTEMIS --ipaddress="${subnet_artemis}" --gateway="${subnet_bridge%.*}.1" --route="${group_number}.0.0.0/8"
                ...
                ./setup/ovs-docker.sh add-port ${group_k}_artemis monitor \
                "${group_number}"_"${rname}"router --ipaddress="${subnet_group}" \
                --macaddress="aa:22:22:22:"$div":"$mod

                ./setup/ovs-docker.sh connect-ports ${group_k}_artemis \
                group_"${group_number}" ${group_k}_ARTEMIS \
                monitor "${group_number}"_"${rname}"router
