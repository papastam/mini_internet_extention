        ...
        for ((i=0;i<n_routers;i++)); do
            ...
            if [ "${group_config}" = "Monitored" ];then
                location="${DIRECTORY}"/groups/g"${group_number}"/"${rname}"/init_conf.sh
                {
                    ...
                    echo " -c 'ip address "$(subnet_router_proxy "${i}" "router")"' \\"
                    echo " -c 'exit' \\"
                    echo " -c 'router ospf' \\"
                    echo " -c '"network "$(subnet_router_proxy "${i}" "router")" area 0"' \\"
                    echo " -c 'exit' \\"
                    ...
                    echo " -c 'exit' \\"
                    echo " -c 'router bgp "${group_number}"' \\"
                    echo " -c 'neighbor "$(subnet_router_proxy "${i}" "proxy")" remote-as $group_number' \\"
                    echo " -c 'neighbor "$(subnet_router_proxy "${i}" "proxy")" route-reflector-client' \\"
                    echo " -c 'neighbor "$(subnet_router_proxy "${i}" "proxy")" route-map LOCAL_PREF_IN_EXA in' \\"
                    echo " -c 'neighbor "$(subnet_router_proxy "${i}" "proxy")" route-map LOCAL_PREF_OUT_EXA out' \\"
                    echo " -c 'exit' \\"
                } >> "${location}"
